...
use anyhow::Context;
use shared::{clickhouse::emote_stat::EmoteStat, database::emote::EmoteId};
...
pub async fn run(
    ...
    clickhouse.query(include_str!("../../../../clickhouse/emote_stats.sql"))
        .execute()
        .await
    ...
    for (emote_id, count) in true_emote_stats {
        let current_count = emote_stats.range((emote_id, time::Date::MIN)..=(emote_id, time::Date::MAX))
            .map(|(_, count)| count)
            .sum::<i32>();
