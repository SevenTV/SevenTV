...
impl<S> IpMiddlewareService<S> {
	fn modify<B>(&mut self, req: &mut Request<B>) -> Result<(), ApiError> {
		let connecting_ip = req
			.extensions()
			.get::<std::net::IpAddr>()
			.ok_or_else(|| ApiError::internal_server_error(ApiErrorCode::Ip, "missing connecting ip address"))?;
		...
		if trusted_proxies.iter().all(|net| !net.contains(connecting_ip)) {
			return Err(ApiError::forbidden(ApiErrorCode::Ip, "ip is not trusted"));
		}
		...
		let ips = req
			.headers()
			.get(header)
			.ok_or_else(|| ApiError::forbidden(ApiErrorCode::Ip, "missing ip header"))?;
		let ips = ips
			.to_str()
			.map_err(|_| ApiError::bad_request(ApiErrorCode::Ip, "ip header is not a valid string"))?;
		let ips = ips.split(',').map(|ip| ip.trim());
		let ips = ips
			.map(|ip| ip.parse::<std::net::IpAddr>())
			.collect::<Result<Vec<_>, _>>()
			.map_err(|_| ApiError::bad_request(ApiErrorCode::Ip, "invalid ip header"))?;
