...
impl From<shared::database::emote::Emote> for Emote {
    fn from(value: shared::database::emote::Emote) -> Self {
        Self {
            id: value.id,
            owner_id: value.owner_id,
            default_name: value.default_name,
            tags: value.tags,
            images: value.image_set.outputs.into_iter().map(Into::into).collect(),
