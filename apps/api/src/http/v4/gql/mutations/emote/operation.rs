...
impl EmoteOperation {
	...
	async fn name(
		...
		self.check_permission(global, session, false).await?;
	...
	async fn flags(&self, ctx: &Context<'_>, flags: EmoteFlagsInput) -> Result<Emote, ApiError> {
		...
		self.check_permission(global, session, requires_manage_any).await?;
	...
	async fn owner(&self, ctx: &Context<'_>, owner_id: UserId) -> Result<Emote, ApiError> {
		...
		self.check_permission(global, session, true).await?;
