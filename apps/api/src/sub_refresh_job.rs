...
async fn handle_xmas_2024_gift(global: &Arc<Global>, customer_id: UserId) -> Result<Xmas2024Gift, ApiError> {
    ...
    const EASTER_4_SUBS_ID: &str = "01964536-a6aa-1202-4ddc-0b2302d470ef";
    const SUMMER_4_SUBS_ID: &str = "01984676-eb69-cafc-dd9f-8162639c25d9";
    const XMAS_2025_4_SUBS_ID: &str = "019b1d83-77b4-81b3-72ef-0f7f6e5aad53";
    const VALENTINE_2026_6_SUBS_ID: &str = "019c531f-2e45-1cb1-d5e4-2400ffa7e7b0";
    const EASTER_2026_4_SUBS_ID: &str = "019d54c9-759c-4062-2f38-ac03504cb4e5";
    ...
    let events = [
        ...
        Event {
            start: chrono::Utc.with_ymd_and_hms(2025, 4, 18, 8, 0, 0).unwrap(),
            end: chrono::Utc.with_ymd_and_hms(2025, 4, 21, 8, 0, 0).unwrap(),
            levels: vec![Level {
                event_id: SpecialEventId::from_str(EASTER_4_SUBS_ID).unwrap(),
                count: 4,
            }],
        },
        Event {
            start: chrono::Utc.with_ymd_and_hms(2025, 7, 27, 20, 0, 0).unwrap(),
            end: chrono::Utc.with_ymd_and_hms(2025, 8, 27, 20, 0, 0).unwrap(),
            levels: vec![Level {
                event_id: SpecialEventId::from_str(SUMMER_4_SUBS_ID).unwrap(),
                count: 4,
            }],
        },
        Event {
            start: chrono::Utc.with_ymd_and_hms(2025, 12, 14, 15, 30, 0).unwrap(),
            end: chrono::Utc.with_ymd_and_hms(2026, 1, 1, 0, 0, 0).unwrap(),
            levels: vec![Level {
                event_id: SpecialEventId::from_str(XMAS_2025_4_SUBS_ID).unwrap(),
                count: 4,
            }],
        },
        Event {
            start: chrono::Utc.with_ymd_and_hms(2026, 2, 13, 9, 0, 0).unwrap(),
            end: chrono::Utc.with_ymd_and_hms(2026, 2, 19, 0, 0, 0).unwrap(),
            levels: vec![Level {
                event_id: SpecialEventId::from_str(VALENTINE_2026_6_SUBS_ID).unwrap(),
                count: 6,
            }],
        },
        Event {
            start: chrono::Utc.with_ymd_and_hms(2026, 4, 4, 11, 0, 0).unwrap(),
            end: chrono::Utc.with_ymd_and_hms(2026, 4, 12, 23, 0, 0).unwrap(),
            levels: vec![Level {
                event_id: SpecialEventId::from_str(EASTER_2026_4_SUBS_ID).unwrap(),
                count: 4,
            }],
        },
    ];
