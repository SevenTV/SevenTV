    ...
    loop {
        ...
        match get_network_devices().await {
            Ok((physical_devices, virtual_devices)) => {
                physical_devices.into_iter().map(|d| (d, true)).chain(virtual_devices.into_iter().map(|d| (d, false))).for_each(|(d, physical)| {
                    let interface: Arc<str> = d.interface.into();
                    system::network_stats(interface.clone(), system::NetworkStatsKind::RxBytes, physical).set(d.rx_bytes as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::RxPackets, physical).set(d.rx_packets as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::RxErrors, physical).set(d.rx_errs as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::RxDropped, physical).set(d.rx_drop as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::TxBytes, physical).set(d.tx_bytes as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::TxPackets, physical).set(d.tx_packets as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::TxErrors, physical).set(d.tx_errs as i64);
                    system::network_stats(interface.clone(), system::NetworkStatsKind::TxDropped, physical).set(d.tx_drop as i64);
                });
                state.network = true;
            },
