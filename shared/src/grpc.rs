...
impl<R: DnsResolver> ChannelController<R> {
	fn new(resolver: R, opts: ChannelControllerOpts) -> Self {
		Self {
			...
			hostnames: opts.hostnames.into_iter().collect(),
			static_ips: opts.static_ips.into_iter().collect(),
	...
	async fn run(&mut self) -> bool {
		let mut addresses = self.static_ips.iter().copied().map(EndpointType::Ip).collect::<HashSet<_>>();
