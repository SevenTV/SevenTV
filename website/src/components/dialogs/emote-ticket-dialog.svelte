<script lang="ts">
	import { ArrowsMerge, Check, EyeSlash, Gavel, NotePencil, PaperPlaneRight, Trash } from "phosphor-svelte";
...
<Dialog bind:mode width={42}>
    <form class="layout">
        ...
        <hr />
        <div class="tabs">
            <TabLink title="Activity Log (17)" matcher={() => tab === "activity"} on:click={() => (tab = "activity")} />
            <TabLink title="Mod Comments (12)" matcher={() => tab === "comments"} on:click={() => (tab = "comments")} />
        </div>
        {#if tab === "activity"}
            Activity
        {:else}
